...
df = pd.read_parquet(f"{SP}/exp006_frame.parquet")
meta = json.load(open(f"{SP}/exp006_meta.json"))
...
json.dump(results, open(f"{SP}/exp006_results.json", "w"), indent=1)
np.savez_compressed(f"{SP}/exp006_arrays.npz", **arrays)
