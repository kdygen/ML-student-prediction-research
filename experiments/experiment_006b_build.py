"""Experiment 006b (sensitivity) — full-course leakage-free dataset builder.
...
win = dh.assign(
    ...
    decay=np.exp(-(dh["horizon"] - dh["date"]).clip(lower=0) / 7.0) * dh["sum_click"],
    third1=(dh["date"] <= dh["horizon"] / 3) * dh["sum_click"],
    third3=(dh["date"] >= 2 * dh["horizon"] / 3) * dh["sum_click"],
...
df["study_spread"] = (df["active_days"].fillna(0) / df["horizon"].clip(lower=1)).clip(0, 1)
df["engagement_decay_ratio"] = df["clicks_last_third"].fillna(0) / (df["clicks_first_third"].fillna(0) + 1)
df["days_since_last"] = df["horizon"] - df["last_day"]
df["days_since_last"] = df["days_since_last"].fillna(999)   # constant never-observed sentinel
df["first_submit_day"] = df["first_submit_day"].fillna(df["course_end"] + 30)
...
df.to_parquet(f"{SP}/exp006b_frame.parquet")
meta = {"n_rows": int(len(df)), "n_cls_features": len(CLS_FEATURES),
        ...
        "audit": audit}
json.dump(meta, open(f"{SP}/exp006b_meta.json", "w"), indent=1)
